...
class HabitStatsViewModel: ObservableObject {
    ...
    let emptyData =  HabitItem(id: 1, image: "💦", title: String(localized: "Drink water"), isQuit: false, backgroundColor: HabitColors.Blue.rawValue, habitFrequency: .daily, isChecked: false, trackCount: 1,trackDates: [Calendar.current.date(byAdding: .day, value: 1, to: .now) ?? .now, Calendar.current.date(byAdding: .day, value: 2, to: .now) ?? .now, Calendar.current.date(byAdding: .day, value: 3, to: .now) ?? .now], completion: "40%", streak: 5)
